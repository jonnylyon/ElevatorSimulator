    ...
    class SimulationConfigLoader
    {
        ...
        private Building generateBuildingFromConfig()
        {
            ...
            foreach (XElement xshaft in xbuilding.Elements("Shaft"))
            {
                ...
                {
                    newShaft.addCar(carAttribs[xcar.Attribute("attributes").Value], int.Parse(xcar.Attribute("startFloor").Value), (CarType)Enum.Parse(typeof(CarType), xcar.Attribute("type").Value), object.ReferenceEquals(xcar.Attribute("parkFloor"), null) ? null : (int?)int.Parse(xcar.Attribute("parkFloor").Value));
                }
            }
